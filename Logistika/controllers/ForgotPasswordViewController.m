...
- (IBAction)clickAction:(UIView*)sender {
    ...
    if (tag == 200) {
        if ([self checkInput1]) {
            ...
            [manager ontemplateGeneralRequest2:params BasePath:FORGOT Path:@"getQuestionFromEmailCorporate" withCompletionBlock:^(NSDictionary *dict, NSError *error) {
                if (error == nil) {
        ...
        if ([self checkInput2]) {
            ...
            NetworkParser* manager = [NetworkParser sharedManager];
            [CGlobal showIndicator:self];
            [manager ontemplateGeneralRequest2:params BasePath:FORGOT Path:@"forgotPasswordEmployer" withCompletionBlock:^(NSDictionary *dict, NSError *error) {
                if (error == nil) {
                    if (dict!=nil && dict[@"result"] != nil) {
                        //
                        if([dict[@"result"] intValue] == 200){

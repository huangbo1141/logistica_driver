...
//                        paths.append(dict["UIImagePickerControllerReferenceURL"])
    ...
    @objc func clickView(view:UIView){
        ...
        case 100:
            ...
            if let navc = self.navigationController, let vc = self.vc {
                if vc is OrderDetailViewController {
                    let ovc = vc as! OrderDetailViewController
                    if self.type == 1 {
                        // shipper
                        if ovc.items_shipper == nil {
                            ovc.items_shipper = NSMutableArray.init()
                        }
                        ovc.items_shipper?.addObjects(from: data)
                    }else if(self.type == 2){
                        if ovc.items_receiver == nil {
                            ovc.items_receiver = NSMutableArray.init()
                        }
                        ovc.items_receiver?.addObjects(from: data)
                    }
                    navc.popViewController(animated: true)
                    return
                }else if vc is OrderDetailCorViewController {
                    let ovc = vc as! OrderDetailCorViewController
                    if self.type == 1 {
                        // shipper
                        if ovc.items_shipper == nil {
                            ovc.items_shipper = NSMutableArray.init()

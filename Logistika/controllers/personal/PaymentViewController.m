...
-(void)showOrderConfirm{
    ...
    if (env.quote) {
        if(env.mode == c_PERSONAL){
            env.order_id = g_quote_id;
            [self update_order:g_quote_id];
        }else{
            env.order_id = g_quote_id;
            [self update_corporate_order:g_quote_id];
        }
    }else{
        [self order];
    }
...
-(void)order{
    ...
    if (g_ORDER_TYPE == g_CAMERA_OPTION) {
        ...
        for (int i=0; i<g_cameraOrderModel.itemModels.count; i++) {
            ...
            if (imageData!=nil) {
                [images addObject:imodel.image_data];
                continue;
            }
...
-(void)update_order:(NSString*)orderid{
    ...
    NetworkParser* manager = [NetworkParser sharedManager];
    [CGlobal showIndicator:self];
    [manager ontemplateGeneralRequest2:params BasePath:BASE_URL Path:@"update_order" withCompletionBlock:^(NSDictionary *dict, NSError *error) {
        if (error == nil) {
            if (dict!=nil && dict[@"result"] != nil) {
                //
                if([dict[@"result"] intValue] == 400){
...
-(void)update_corporate_order:(NSString*)orderid{
    ...
    NetworkParser* manager = [NetworkParser sharedManager];
    [CGlobal showIndicator:self];
    [manager ontemplateGeneralRequest2:params BasePath:BASE_URL Path:@"update_corporate_order" withCompletionBlock:^(NSDictionary *dict, NSError *error) {
        if (error == nil) {
            if (dict!=nil && dict[@"result"] != nil) {
                //
                if([dict[@"result"] intValue] == 400){

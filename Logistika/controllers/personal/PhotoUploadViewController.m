...
- (void)viewDidLoad {
    ...
    _imgCell1.hidden = true;
    _imgCell2.hidden = true;
    _imgCell3.hidden = true;
    
    _imgCell1.aDelegate = self;
    _imgCell2.aDelegate = self;
    _imgCell3.aDelegate = self;
...
-(void)clickView:(UIView*)sender{
    ...
    switch (tag) {
        case 100:
        {
            NSArray* cells = @[self.imgCell1,self.imgCell2,self.imgCell3];
            NSMutableArray* data = [[NSMutableArray alloc] init];
            for (int i=0; i<_limit; i++) {
                SelectImageCell* cell = cells[i];
...
- (IBAction)launchController
{
    ...
    elcPicker.maximumImagesCount = _limit - _index; //Set the maximum number of images to select to 100
...
-(IBAction)captureNow
{
    if (self.index<self.limit) {
        if (self.theImage!=nil) {
            ...
            SelectImageCell* cell = cells[self.index];
            ...
            [cell setStyleWithData:@{@"image":self.theImage,@"path":filePath} Mode:self.index];
...
- (void)elcImagePickerController:(ELCImagePickerController *)picker didFinishPickingMediaWithInfo:(NSArray *)info
{
    ...
    if (_index<_limit) {
        ...
        for(int i=0; i<[images count]; i++){
            ...
            [cell setStyleWithData:@{@"image":images[i],@"path":path} Mode:self.index];
            _index = _index + 1;
            if (_index>=_limit) {
                break;
            }
...
-(void)didSubmit:(id)obj View:(UIView *)view{
    if ([obj isKindOfClass:[NSDictionary class]]) {
        ...
        int index = [dict[@"mode"] intValue];
        NSArray* cells = @[self.imgCell1,self.imgCell2,self.imgCell3];
        for (int i=index; i<_limit; i++) {
            SelectImageCell* cell = cells[i];
            cell.hidden = true;
        }
        self.index = self.index - 1;
